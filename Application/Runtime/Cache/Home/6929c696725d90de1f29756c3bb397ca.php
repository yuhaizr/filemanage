...
<body class="cbp-spmenu-push">
	...
	<div class='tables'>
		...
		<div class="bs-example widget-shadow" data-example-id="bordered-table"> 
			<h4>档案模板列表</h4>
			<form action="/filemanage/index.php/Home/File/showList" method="GET">
			
				<div class='row'>
					<div class='col-sm-3'>
							<select id="file_temp_id" name='file_temp_id'  style="width:100%;">
						 		<option value="" />请选择你要添加的模板
						 		<?php if(is_array($fileTempList)): foreach($fileTempList as $key=>$vo): ?><option <?php if($vo['id'] == $file_temp_id ): ?>selected="delected"<?php endif; ?> value="<?php echo ($vo["id"]); ?>" /><?php echo ($vo["title"]); endforeach; endif; ?>
						 	</select> 
							
					</div>
					<div class='col-sm-3'>
						<input type="text"  name="searchValue" value="<?php echo ($searchValue); ?>" id="searchValue" class="form-control nameinput" placeholder="请输入您要搜索的内容"> 
					</div>
					<div class='col-sm-2'>
			...
			<table class="table table-bordered"> 
					<thead> 
						<tr> 
						 <?php if(!empty($list)): ?><th>序号</th> 
							<th>创建时间</th> 
							<?php if(is_array($tempinfo["name"])): foreach($tempinfo["name"] as $key=>$vo): ?><th><?php echo ($vo); ?>(<?php echo ($key); ?>)</th><?php endforeach; endif; ?>
							<th>操作</th><?php endif; ?>
						</tr> 
					</thead> 
					<tbody> 
						<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr> 
            				...
            				<?php if(is_array($vo["value"])): foreach($vo["value"] as $k=>$v): ?><td><?php echo ($v); ?></td><?php endforeach; endif; ?>
           						 
        
                			<td>
                	
                			
                			<a <?php if($my_detail_btn != 1): ?>style="display: none;"<?php endif; ?> href="/filemanage/index.php/Home/File/detail?type=menu&id=<?php echo ($vo["id"]); ?>" data-bb-handler="confirm" type="button"  class=" my_detail_btn btn btn-primary">详情</a>
                		
           					
           			
           					<a <?php if($my_modify_btn != 1): ?>style="display: none;"<?php endif; ?> href="/filemanage/index.php/Home/File/modify?type=menu&id=<?php echo ($vo["id"]); ?>" data-bb-handler="confirm" type="button"  class=" my_modify_btn btn btn-primary">修改</a>
           		
           				
           					<a <?php if($my_del_btn != 1): ?>style="display: none;"<?php endif; ?> href="/filemanage/index.php/Home/File/del?type=menu&id=<?php echo ($vo["id"]); ?>" data-bb-handler="confirm" type="button"    class=" my_del_btn btn btn-danger  delete">删除</a>
